...
class _AgendamientoDashboardState extends State<AgendamientoDashboard> {
  ...
  void _procesarCita(CentroVacunacion centro, Paciente paciente, String idTramo, DateTime fechaCita) {
    ...
    if (cita != null) {
      CustomDialogs.showMessage(context, "Éxito", "Cita creada exitosamente en ${centro.nombre} para el ${DateFormatter.formatDateTime(cita.fechaHora)}");
      setState(() {
        _fechaCitaSeleccionada = null;
      }); 
    } else {
      CustomDialogs.showMessage(context, "Error", "No hay disponibilidad en el centro para la fecha seleccionada.");
    }
  }
        ...
        children: [
          ...
          Card(
            ...
            child: ListTile(
              ...
              title: Text(_fechaCitaSeleccionada == null 
          ...
          ElevatedButton.icon(
            onPressed: (_tramoSeleccionado == null || _centroSeleccionado == null || _fechaCitaSeleccionada == null) ? null : () {
              bool tienePrioridad = _tramoSeleccionado!.validarPrioridadPaciente(pacienteObjetivo);

              if (tienePrioridad) {
                _procesarCita(_centroSeleccionado!, pacienteObjetivo, _tramoSeleccionado!.idTramo, _fechaCitaSeleccionada!);
              } else {
                showDialog(
                  context: context,
                  builder: (ctx) => AlertDialog(
                    title: const Text("Advertencia de Prioridad", style: TextStyle(fontWeight: FontWeight.bold)),
                    content: Text(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    child: ListTile(
                      ...
                      title: Text(DateFormatter.formatDateTime(cita.fechaHora)),
